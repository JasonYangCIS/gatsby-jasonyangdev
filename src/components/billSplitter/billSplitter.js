...
class BillSplitter extends Component {
  ...
  personTotal(
    ...
    let billTax = 0.0;
    let billTip = 0.0;
    let billSubTotal = 0.0;
    let billGrandTotal = 0.0;
  ...
  render() {
    ...
    for (var i = 0; i < this.state.numPeople; i++) {
      children.push(
        <Person
          key={i}
          number={i}
          name={this.state.peopleArray[i]}
          tax={this.state.taxPercentage}
          tip={this.state.tipPercentage}
          personTotal={this.personTotal}
        />
      );
    }
    ...
    return (
      <div className="bill-splitter">
        ...
        <GrandTotal
          billTax={this.state.billTax}
          billTip={this.state.billTip}
          billSubTotal={this.state.billSubTotal}
          billGrandTotal={this.state.billGrandTotal}
        />
      </div>
    );
  }
...
class Person extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      itemName: [],
      itemCost: [],
      subTotalItemCost: 0.0,
      totalTaxCost: 0.0,
      totalTipCost: 0.0,
      grandTotalItemCost: 0.0,
      active: true,
    };
  ...
  calculateCosts() {
    ...
    for (var i = 0; i < this.state.numItems; i++) {
      ...
      totalTaxCost = (
        subTotalItemCost * parseFloat(this.props.tax * 0.01)
      ).toFixed(2);
      totalTipCost = (
        subTotalItemCost * parseFloat(this.props.tip * 0.01)
      ).toFixed(2);
      grandTotalItemCost = (
        parseFloat(subTotalItemCost) +
        parseFloat(totalTaxCost) +
        parseFloat(totalTipCost)
      ).toFixed(2);
    }
    ...
    for (var i = 0; i < this.state.numItems; i++) {
      items.push(
        <Item
          key={i}
          number={i}
          itemName={this.state.itemName[i]}
          itemCost={parseFloat(this.state.itemCost[i]).toFixed(2)}
        />
      );
    }
    ...
    if (this.state.active === true) {
      return (
        <div className="person-card">
          ...
          <div className="remove" onClick={this.removePerson}>
            X
          </div>
      ...
      return (
        <div className="grand-total-container">
          <div className="sub-total">
            {" "}
            <span>Sub Total:</span>{" "}
            <span>{this.props.billSubTotal.toFixed(2)} </span>{" "}
          </div>
          <div className="grand-tax">
            {" "}
            <span>Total Tax:</span>{" "}
            <span>{this.props.billTax.toFixed(2)} </span>{" "}
          </div>
          <div className="grand-tax">
            {" "}
            <span>Total Tip:</span>{" "}
            <span>{this.props.billTip.toFixed(2)} </span>{" "}
          </div>
          <div className="grand-total">
            <span>Grand Total:</span>{" "}
            <span>${this.props.billGrandTotal.toFixed(2)}</span>{" "}
          </div>
        </div>
      );
    } else {
