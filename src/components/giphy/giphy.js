...
class Gifs extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.keyword !== prevProps.keyword) {
      ...
      let giphyApi =
        ...
        keyword +
        "&api_key=pH4317HZgmQsVE6tFSkfH8pdXIG13lAC&limit=6";
      fetch(giphyApi)
        .then(res => res.json())
        .then(
          result => {
  ...
  copyGiphyUrl(text) {
    if (
      ...
      try {
        document
          .getElementById("copied-notification")
          .classList.add("show-copied");
        setTimeout(function() {
          document
            .getElementById("copied-notification")
            .classList.remove("show-copied");
        }, 2000);
  ...
  render() {
    ...
    if (this.state.hasGiphyResults && this.state.giphyList.length > 0) {
      this.state.giphyList.forEach(function(item) {
        ...
        images.push(
          <li key={key} onClick={() => this.copyGiphyUrl(embedUrl)}>
      ...
      return (
        <div className="giphy-list-container">
          <ul id="giphy-list">{images}</ul>
          <div id="copied-notification">Gif URL Copied!</div>
        </div>
      );
    } else if (
      this.props.keyword === "Enter search term." ||
      this.state.giphyList.length === 0
    ) {
      return (
        <div>
